...
export default function CTASection() {
  ...
  return (
    <div className="relative rounded-3xl bg-slate-900 border border-slate-800 text-white p-8 sm:p-12 overflow-hidden shadow-xl my-12 select-none">
      ...
      <div className="relative flex flex-col md:flex-row items-center justify-between gap-8 z-10 text-center md:text-left">
        ...
        <div className="space-y-4 max-w-xl">
          ...
          <h2 className="font-serif text-2xl sm:text-4xl font-bold tracking-tight text-white leading-tight">
            Start building clarity, <br className="hidden sm:inline" />
            one note at a time.
          </h2>
        ...
        {/* Right Action Button with spring physics */}
        <div className="shrink-0 w-full md:w-auto">
          <Link href="/notes" className="w-full md:w-auto">
            <motion.div
              whileHover={{ scale: 1.03 }}
              whileTap={{ scale: 0.97 }}
              transition={springTransition}
              className="w-full md:w-auto inline-flex items-center justify-center space-x-2 px-8 py-4 rounded-2xl bg-amber-500 hover:bg-amber-605 text-slate-950 font-bold transition-shadow shadow-md hover:shadow-lg cursor-pointer"
            >
