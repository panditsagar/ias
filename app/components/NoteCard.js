...
export default function NoteCard({ note }) {
  ...
  return (
    <motion.div
      ...
      className="group bg-white rounded-2xl border border-slate-200/60 shadow-[0_20px_40px_-15px_rgba(0,0,0,0.05)] transition-colors duration-300 hover:border-amber-300/50 flex flex-col justify-between overflow-hidden"
    >
      {/* Upper Content Area */}
      ...
      <div className="px-6 sm:px-8 py-4 border-t border-slate-100 bg-slate-50/50 flex items-center justify-between text-xs text-slate-400">
        ...
        <Link
          href={`/notes/${id}`}
          className="inline-flex items-center space-x-0.5 font-semibold text-amber-700 hover:text-amber-850 transition-colors group-hover:underline active:scale-[0.98] transition-transform"
        >
          <span>Read Notes</span>
          <CaretRight className="h-3.5 w-3.5 transform group-hover:translate-x-0.5 transition-transform" weight="bold" />
        </Link>
      </div>
    </motion.div>
  );
